...
- `widgets/<name>/widget.pudding-card.json`：可安装的运行快照。
...
4. 写回 `widgets/<name>/widget.pudding-card.json`。
...
`source/` 给维护者编辑；Pudding 安装的是 `widget.pudding-card.json`。

除非调试打包产物，不要手改 `widget.pudding-card.json`。应修改源码，然后重新运行打包命令。
