...
- 运行快照：`widgets/<name>/widget.pudding-card.json`，由打包脚本生成，Pudding 安装它。
...
4. 压缩运行 HTML 快照。
5. 写入 `widget.pudding-card.json`。
6. 更新 `manifest.json` 和 `widgets/registry.json` 中的 `card_sha256`。

不要手动编辑 `widget.pudding-card.json`。应该修改 `source/`，然后重新打包。
...
- 打包后 `widget.pudding-card.json` 已更新。
...
核心原则：源码给人维护，`widget.pudding-card.json` 给 Pudding 安装；小组件自己管理业务状态，Pudding 只提供身份、会话、主题、语言和消息通道。
