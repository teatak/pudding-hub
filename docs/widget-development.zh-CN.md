...
- `requires.widget_api`：兼容的 Pudding widget API 范围。
...
4. 压缩运行 HTML 快照。
5. 写入 `releases/<widget_version>/<name>.pudding-widget.json`。
6. 将 assets、source、screenshots 和 manifest 复制到 release 目录。
7. 更新 `manifest.json` 和 `widgets/registry.json` 中的 `package_sha256`。

不要手动编辑 `releases/<version>/` 下的文件。应该修改 `source/`，必要时提升 `widget_version`，然后重新打包。
...
- 打包后 `releases/<widget_version>/` 已更新。
