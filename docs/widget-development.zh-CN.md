...
- 运行快照：`widgets/<name>/widget.pudding-widget.json`，由打包脚本生成，Pudding 安装它。

维护者修改源码，再重新生成运行快照。Pudding 安装的是运行快照。
...
`source/index.html` 是入口文件。打包时会把这个文件引用的本地 CSS 和 JS 内联进运行快照。
...
4. 压缩运行 HTML 快照。
5. 写入 `widget.pudding-widget.json`。
6. 更新 `manifest.json` 和 `widgets/registry.json` 中的 `card_sha256`。

不要手动编辑 `widget.pudding-widget.json`。应该修改 `source/`，然后重新打包。
...
- 打包后 `widget.pudding-widget.json` 已更新。
- `manifest.json` 和 `widgets/registry.json` 中的 `card_sha256` 已更新。
- 小组件在亮色和暗色主题下都可用。
...
核心原则：源码给人维护，`widget.pudding-widget.json` 给 Pudding 安装；小组件自己管理业务状态，Pudding 只提供身份、会话、主题、语言和消息通道。
